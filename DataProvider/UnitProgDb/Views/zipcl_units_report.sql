CREATE VIEW dbo.zipcl_units_report
AS
SELECT     TOP (100) PERCENT
                          (SELECT     TOP (1) name_inn
                            FROM          dbo.get_contractor(c.id_contractor) AS get_contractor_1) AS Контрагент, cu.catalog_num AS [Каталожный номер], cu.nomenclature_num AS [Номенклатурный номер], 
                      cu.name AS Наименование, cu.count AS Количество, cu.price_in AS [Цена ВХОД], cu.price_out AS [Цена ВЫХОД],
                          (SELECT     COUNT(1) AS Expr1
                            FROM          dbo.zipcl_claim_units AS clu
                            WHERE      (enabled = 1) AND (cu.id_claim = id_claim)) AS [Количество ЗИП в данной заявке], cu.id_claim AS [ID Заявки], CONVERT(NVARCHAR, c.dattim1, 104) AS [Дата Заявки],
                          (SELECT     name
                            FROM          dbo.zipcl_claim_states AS st
                            WHERE      (id_claim_state = c.id_claim_state)) AS [Текущий статус заявки], c.id_device, ISNULL(dbo.srvpl_fnc(N'getDeviceShortCollectedNameNoSerialNoBr', NULL, c.id_device, NULL, NULL), 
                      ISNULL(c.device_model, '')) AS Модель, c.city AS Город, c.address AS Адрес,
                          (SELECT     name
                          ...
                          (SELECT     display_name
                            FROM          dbo.users AS u
                            WHERE      (id_user = c.id_manager)) AS Менеджер, c.serial_num AS [Серийный номер],
                          ...
                          (SELECT     (CASE WHEN cc.old_id_claim IS NULL THEN ISNULL
                            ...
                            WHERE      (cc.id_claim_state =
                                                       (SELECT     id_claim_state
                                                         FROM          dbo.zipcl_claim_states AS zipcl_claim_states_6
                                                         WHERE      (enabled = 1) AND (sys_name = 'DONE')))) AS Завершена,
                          (SELECT     (CASE WHEN cc.old_id_claim IS NULL THEN ISNULL
                                                       ((SELECT     TOP 1 ccc.dattim1
                                                           FROM         dbo.zipcl_zip_claims ccc
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[zipcl_units_report] TO [UN1T\Alexandr.Medvedevski]
    AS [dbo];


GO
GRANT SELECT
    ON OBJECT::[dbo].[zipcl_units_report] TO [UN1T\sergey.kokoulin]
    AS [dbo];
...
Begin DesignProperties = 
   Begin PaneConfigurations = 
      Begin PaneConfiguration = 0
         NumPanes = 4
         Configuration = "(H (1[20] 4[12] 2[48] 3) )"
      End
   ...
   Begin CriteriaPane = 
      Begin ColumnWidths = 11
         ...
         Filter = 1350
         Or = 1350
         Or =', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'zipcl_units_report';
...
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPane2', @value = N' 1350
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'zipcl_units_report';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 2, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'zipcl_units_report';
