...
GO
ALTER PROCEDURE [dbo].[get_question] @id INT = NULL, @manager_sid VARCHAR(46) = NULL, @lst_que_states NVARCHAR(100) = NULL
AS
    BEGIN
        SET NOCOUNT ON;
        SELECT  id, manager_sid, date_limit, descr , id_que_state, que_state, dattim1
        ...
        WHERE   ( @id IS NULL
					...
					AND ( (@manager_sid IS NULL OR @manager_sid = '')
                      OR ( @manager_sid IS NOT NULL
                           AND @manager_sid != ''
                           AND q.manager_sid = @manager_sid
                         )
                    )
					AND ( (@lst_que_states IS NULL OR @lst_que_states = '')
                      OR ( @lst_que_states IS NOT NULL
                           AND @lst_que_states != ''
                           AND q.id_que_state in (SELECT value from Split(@lst_que_states, ','))
                         )
                    )
					order by id desc
    END
