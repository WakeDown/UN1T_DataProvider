...
GO
ALTER TABLE [dbo].[photos] ALTER COLUMN [picture] IMAGE NULL;
...
AS
    BEGIN
        ...
        SELECT  e.id ,
                ...
                END AS photo,
				CASE WHEN @get_photo = 1 THEN 
				(select top 1 convert(nvarchar(max),picture, 2) from photos ph where ph.enabled=1 and ph.id_employee=e.id)
                     ELSE null
                END AS photo_name
        FROM    employees e
