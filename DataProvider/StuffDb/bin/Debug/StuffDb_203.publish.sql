...
((@id_department IS NULL OR @id_department <= 0 OR (@id_department IS NOT NULL AND @id_department > 0 AND EXISTS(SELECT 1 FROM document_meet_links dml WHERE dml.enabled=1 and dml.id_document = d.id AND dml.id_department = @id_department)))
