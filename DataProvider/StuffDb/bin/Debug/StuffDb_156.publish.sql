...
ALTER TABLE [dbo].[positions]
    ADD [name_rod] NVARCHAR (500) NULL;


GO
PRINT N'Выполняется создание [dbo].[org_state_images]...';
...
GO
PRINT N'Выполняется создание ограничение без названия для [dbo].[org_state_images]...';
...
AS
	...
	SET NOCOUNT ON;
        IF @id IS NOT NULL
            ...
            BEGIN
                UPDATE  positions
                SET     name = @name , name_rod=@name_rod
                WHERE   id = @id
            END
        ELSE
            BEGIN
                INSERT  INTO positions
                        ( name  ,creator_sid, name_rod
                        )
                VALUES  ( @name  ,@creator_sid, @name_rod
                        )
    ...
    END
GO
PRINT N'Выполняется создание [dbo].[close_org_state_image]...';
...
AS
    ...
    END
GO
PRINT N'Выполняется создание [dbo].[get_org_state_image]...';
...
GO
PRINT N'Выполняется создание [dbo].[save_org_state_image]...';
