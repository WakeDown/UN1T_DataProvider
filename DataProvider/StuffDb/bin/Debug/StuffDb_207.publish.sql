...
--текущий год
	...
	--т.к. не уверен на каком языке будет выполняться данная функция, то хардкодим пятницу, субботу и воскресенье
        SET @friday_name = DATENAME(weekday, convert(date,'04.10.2013'))
-- точно пятница
        SET @saturday_name = DATENAME(weekday, convert(date,'05.10.2013'))
-- точно суббота
        SET @sunday_name = DATENAME(weekday, convert(date,'06.10.2013'))
-- точно воскресенье
