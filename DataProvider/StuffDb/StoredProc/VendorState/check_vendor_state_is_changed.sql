...
where vs.id=@id AND vs.id_vendor = vs_old.id_vendor AND LTRIM(RTRIM(vs.descr))=RTRIM(LTRIM(vs_old.descr)) AND vs.date_end=vs_old.date_end AND vs.id_organization = vs_old.id_organization AND vs.id_language=vs_old.id_language AND vs.pic_data=vs_old.pic_data)
begin
	select null
end
else begin
