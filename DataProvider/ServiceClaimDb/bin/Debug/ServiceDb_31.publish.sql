...
The column [dbo].[classifier].[id_parent] is being dropped, data loss could occur.

The column [dbo].[classifier].[name] is being dropped, data loss could occur.

The column [dbo].[classifier].[number] is being dropped, data loss could occur.
*/
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[classifier])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_classifier] ([id_category], [dattim1], [dattim2], [enabled], [creator_sid], [id_work_type], [time], [price], [cost_people], [cost_company])
        SELECT [id_category],
               [dattim1],
               [dattim2],
               [enabled],
...
GO
PRINT N'Altering [dbo].[get_classifier_list]...';


GO
ALTER PROCEDURE [dbo].[get_classifier_list]
AS
	begin
	select null
